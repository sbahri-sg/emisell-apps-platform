...
export function createLocalProductReader({ coreOrigin, appId, clientId, secretFile, publicDirectory } = {}) {
  ...
  const endpoint = new URL('/v1/app-platform/core/reviewed-ui/products', coreOrigin);
  ...
  return async (token, { signal, query = {} } = {}) => {
    ...
    const filters = productQuery(new URLSearchParams(query));
    let secret;
    ...
    try { return productPage(value, filters.limit); } catch { throw new IdentityError('verifier_unavailable'); }
  };
}
