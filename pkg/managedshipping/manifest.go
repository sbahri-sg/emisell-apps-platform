...
func (m Manifest) Validate() error {
	...
	if m.Schema != Schema || m.Policy != Policy || !identifier.MatchString(m.AppID) ||
		...
		m.Capability != "shipping/v1" || !slices.Equal(m.Scopes, []string{"shipping.read"}) ||
		m.Binding != (Binding{Engine: "api-kurir", ProviderCode: "emisell"}) || m.Pricing != "free" {
...
func Verify(p Package, key ed25519.PublicKey) error {
	...
	if len(key) != ed25519.PublicKeySize || p.KeyID != KeyID(key) || p.SHA256 != Digest(raw) ||
		!ed25519.Verify(key, append([]byte(Policy+"\n"), raw...), p.Signature) {
