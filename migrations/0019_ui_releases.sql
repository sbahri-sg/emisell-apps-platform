-- UI-only authoring; does not create assignments, installations or grants.
CREATE TABLE platform_app.ui_releases (
 id text PRIMARY KEY, app_id text NOT NULL, organization_id text NOT NULL,
 version text NOT NULL, document jsonb NOT NULL,
 UNIQUE(app_id,version)
);
CREATE TABLE platform_app.ui_release_requests (
 actor_id text NOT NULL, request_key text NOT NULL, request_hash text NOT NULL,
 release_id text NOT NULL REFERENCES platform_app.ui_releases(id),
 PRIMARY KEY(actor_id,request_key)
);
CREATE TABLE platform_app.ui_release_audit (
...
CREATE FUNCTION platform_app.protect_ui_release() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
 ...
 RETURN NEW;
END $$;
CREATE TRIGGER ui_release_immutable BEFORE UPDATE ON platform_app.ui_releases
 FOR EACH ROW EXECUTE FUNCTION platform_app.protect_ui_release();
